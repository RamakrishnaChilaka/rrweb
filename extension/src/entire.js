...
function StartFunction() {
  ...
  return createSession().then(data => {
    ...
    // for tab change
    // TODO: add a condition to check if localStorage exists
    ...
    postToBackend_v1();
    return window.stopFn;
  });
}
// start.js

setTimeout(() => {
  window.stopFn = StartFunction();
}, 5000);

setTimeout(() => {
  StopFunction(window.stopFn);
}, 20 * 2000);

// stop.js
function StopFunction(stopFn) {
  console.log('inside stopFn ', window.stopFn);
