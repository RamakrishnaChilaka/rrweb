...
function meta({ replayer, playbackTime }) {
  return {
...
var methods = {
  ...
  play() {
    ...
    console.log("inside play replayer and currentTime is ", replayer, currentTime, this.get());
    if (currentTime > 0) {
  ...
  pause() {
    console.log("inside sessionPlayer pause");
    const { replayer } = this.get();
    replayer.pause();
  },
  toggle() {
    console.log("inside toggle pressed toggle ");
    console.log("inside toggle this.playing ", this.get().isPlaying, this.get());
  ...
  handleProgressClick(event) {
    ...
    console.log("Percent handleProgressClick ", percent);
...
function onupdate({ changed, current, previous }) {
  console.log("inside onupdate is ", JSON.stringify(changed), current, previous );
...
function Controller(options) {
	...
	this._recompute({ replayer: 1, playbackTime: 1, currentTime: 1, meta: 1 }, this._state);
	if (!('replayer' in this._state)) console.warn("<Controller> was created without expected data property 'replayer'");
	if (!('playbackTime' in this._state)) console.warn("<Controller> was created without expected data property 'playbackTime'");
...
Controller.prototype._recompute = function _recompute(changed, state) {
	if (changed.replayer || changed.playbackTime) {
...
var methods$1 = {
  updateScale(el, frameDimension) {
    ...
    el.style.transform =
      `scale(${Math.min(widthScale, heightScale, 1)})` + 'translate(-50%, -50%)';
  },
  fullscreen() {
    if (this.refs.player) {
...
function oncreate() {
  ...
  replayer.on('resize', dimension => this.updateScale(replayer.wrapper, dimension));
  this.set({
    replayer,
  });
  window.replayerWebPlayer = replayer;
...
function create_if_block$1(component, ctx) {
	...
	return {
		...
		p: function update(changed, ctx) {
			...
			if (changed.playbackTime) controller_changes.playbackTime = ctx.playbackTime;
...
function Player(options) {
	...
	if (!('playbackTime' in this._state)) console.warn("<Player> was created without expected data property 'playbackTime'");
